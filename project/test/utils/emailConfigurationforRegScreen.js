  ...
  return new Promise((resolve, reject) => {
    imap.once('ready', () => {
      imap.openBox('INBOX', false, (err) => {
        if (err) throw err;
        imap.search(['UNSEEN', ['SUBJECT', 'Madelyne email verification']], (error, results) => {
          const messages = [];
          ...
          imapFetch.once('end', () => {
            ...
            for (const i in messages) {
              ...
              if (body.includes('https')) {
                ...
                console.log('The code in the URL:' + splitofurl[1]);
                fs.writeFile('./test/testdata/codeforregscreen.txt', splitofurl[splitofurl.length - 1], { flag: 'w' }, (err2) => {
                  if (err2) throw err2;
...
function readforresendregscreen() {
  ...
  return new Promise((resolve, reject) => {
    imap.once('ready', () => {
      imap.openBox('INBOX', false, (err) => {
        if (err) throw err;
        imap.search(['UNSEEN', ['SUBJECT', 'Please verifiy your email for Madelyne']], (error, results) => {
          const messages = [];
          if (error) throw error;
          const imapFetch = imap.fetch(results, {
            id: 1,
          ...
          imapFetch.once('end', () => {
            ...
            for (const i in messages) {
              ...
              if (body.includes('https')) {
                ...
                const splitofurl = tostringval.split(',');
                console.log('The code in the URL:' + splitofurl[1]);
                fs.writeFile('./test/testdata/codeforregscreen.txt', splitofurl[1], { flag: 'w' }, (err2) => {
                  if (err2) throw err2;
                  console.log('File has been created & saved successfully!');
                });
              } else {
